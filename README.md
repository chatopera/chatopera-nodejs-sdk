...
[Chatopera](https://www.chatopera.com/)提供聊天机器人开发者平台，Chatopera SDK用于在Node.js应用中集成[聊天机器人服务](https://bot.chatopera.com/)。
...
通过[示例程序](https://github.com/chatopera/chatopera-sample-node)快速开始使用SDK。
...
说明：返回值为数组，*id*代表问答对ID，*score*是相似度分数，*reply*是回复。在含有多个元素时按*score*降序排列，*score*的分数在[0,1]之间，越接近1越相似。
...
以该用户ID发送给聊天机器人信息时，将不能得到回复，此时回复内容[参考文档](https://docs.chatopera.com/chatbot-engine.html)。
...
```
其中，`ClientID`和`secret`来自于平台，在独立使用智能问答引擎时，可以省略`secret`。`userName`代表用户名，是一个不含空格或特殊符号的字符串，每个用户的唯一标识。
...
其中`botarchive`为机器人的文件目录，支持*相对路径*或*绝对路径*。
